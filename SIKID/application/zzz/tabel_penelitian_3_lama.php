...
if(isset($_POST['tahun'])&&$_POST['tahun']=='All'&&isset($_POST['status'])&&$_POST['status']=='Status'){ 
	...
	$st=$_POST['status'];
	$datanya = $this->db->query("select * FROM penelitian where status='$st' order by tahun desc")->result_array();
}else if (isset($_POST['tahun'])&&$_POST['tahun']!='All'&&isset($_POST['status'])&&$_POST['status']=='Status'){
	$thn=$_POST['tahun'];
	...
	$thn=$_POST['tahun'];
	$st=$_POST['status'];
	$datanya = $this->db->query("select * FROM penelitian WHERE tahun=$thn and status='$st'")->result_array();
}
?>
			...
			for ($i=0;  $i<$jumlah; $i++){
				...
				$jml=$jml+$data['alokasi_dana'];
					echo "<tr>
						 ...
						 <a class='btn btn-minier  btn-info' href='".site_url('c_jurusan/detail_penelitian/'.$data['idpenelitian'])."'>Detail</a>
