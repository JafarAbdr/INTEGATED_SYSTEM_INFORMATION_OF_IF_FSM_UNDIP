	...
	<div class="alert alert-error">
		...
		<?php echo $this->session->flashdata('alert');
			 //$this->session->unset_flashdata('alert');
			 //$this->session->set_flashdata('message',0);
		?>
	</div>	
	<?php } ?>
	
	<?php if ($this->session->flashdata('message')==2) { ?>
	<div class="alert alert-error">
		<button type="button" class="close" data-dismiss="alert"><i class="icon-remove"></i></button>
		<h4>Error</h4>
		<?php echo $this->session->flashdata('alert');
	...
	<?php $attributes = array('class' => 'form-horizontal');
		echo form_open('c_dosen/update_password', $attributes);
	?>
	...
	<div class="control-group">
		<label class="control-label" for="kp_baru">Konfirmasi Password</label>
		<div class="controls">
		 <input type="password" name="kp_baru" placeholder="Ketik Ulang Password"/>
		</div>
	</div>
