...
if(isset($_POST['tahun'])&&$_POST['tahun']=='All'&&isset($_POST['kategori'])&&$_POST['kategori']=='kategori'){ 
	...
	$st=$_POST['kategori'];
	$datanya = $this->db->query("select * FROM prosiding where kategori='$st' order by tahun desc")->result_array();
}else if (isset($_POST['tahun'])&&$_POST['tahun']!='All'&&isset($_POST['kategori'])&&$_POST['kategori']=='kategori'){
	$thn=$_POST['tahun'];
	...
	$thn=$_POST['tahun'];
	$st=$_POST['kategori'];
	$datanya = $this->db->query("select * FROM prosiding WHERE tahun=$thn and kategori='$st'")->result_array();
}
?>
			...
			for ($i=0;  $i<$jumlah; $i++){
				...
				$dtr=$dtr+1;
					...
					echo "<tr>
						 ...
						 <td>					 
							<a class='btn btn-minier  btn-info' href='".site_url('c_jurusan/detail_prosiding/'.$data['idprosiding'])."'>Detail</a>
						 </td>
						 </tr>
					";
			}
