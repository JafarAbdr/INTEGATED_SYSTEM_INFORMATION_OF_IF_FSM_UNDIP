<?php
   class Mod_dosennonif extends CI_Model{
      function get_all_dosen(){
         return $this->db->get('tb_dosen_non_if')->result();
      }
      function tambah_dosen($data){
         return $this->db->insert('tb_dosen_non_if',$data);
      }
		function get_id_dosen(){
			
			$idMax = $this->db->query("SELECT max(id_dosen) as id_dosen_max from tb_dosen_non_if")->row()->id_dosen_max; 
      ...
      function update($id_dosen,$data) {
        ...
        $this->db->update('tb_dosen_non_if', $data);
      }
      function getById($id_dosen){
         return $this->db->get_where('tb_dosen_non_if', array('id_dosen' => $id_dosen))->row();
      }
      function delete_data($id_dosen){
         $kal = 'DELETE FROM `tb_dosen_non_if` WHERE id_dosen="'.$id_dosen.'"';
         $this->db->query($kal);
			...
			for($i = 1; $i < count($excel_data); $i++)
			  {            
					if( ! $this->is_exist($excel_data[$i]['nip']) == TRUE)
					{
						 ...
						 $this->db->insert('tb_dosen_non_if', $data);
					}
					else
					{
						 $data = array(                   
		 ...
		 */      
		 private function is_exist($nip)
		 {
			  ...
			  $result = $this->db->count_all_results('tb_dosen_non_if');
			  if($result == 1)
			  {
					return TRUE;
			  }
