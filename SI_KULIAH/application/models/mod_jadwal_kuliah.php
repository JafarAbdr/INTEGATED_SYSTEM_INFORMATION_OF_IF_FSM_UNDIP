<?php
   ...
   class mod_jadwal_kuliah extends CI_Model{
	
      function tambah_data($data){
         $this->db->insert('tb_jadwal_kuliah',$data);
         //$this->db->insert('tb_jadwal_kuliah_rel',$data);
      }
      function get_all_jadwal(){
         return $this->db->get('tb_jadwal_kuliah')->result();
      }
      function get_jadwal_by_id($id_jadwal){
         return $this->db->get_where('tb_jadwal_kuliah', array('id_jadwal' => $id_jadwal))->row();
      }
      ...
      function get_jadwal_for_view(){
         $q = $this->db->query("SELECT b.id_jadwal, b.thn_ajar, b.hari, b.semester, b.jammulai, b.jamselesai, k.nama_ruang, mk.nama_matkul, mk.sks, mk.kode_matkul, mk.semesterke, b.kelas
               FROM tb_jadwal_kuliah as b inner join tb_matkul as mk inner join tb_ruang as k
               on b.matkul = mk.id_matkul and b.ruang = k.id_ruang order by thn_ajar, field(hari, 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu', 'Minggu'), jammulai, jamselesai");
         return $q->result_array();
      }
