<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
   class Con_dosennonif extends CI_Controller{
		public function __construct(){
			...
			if(ISSET($u1) && ISSET($u2) && ISSET($u3) && ($u1 != "") && ($u2 != "") && ($u3 != "")){
				$this->load->model('mod_dosennonif'); 
			}else{
				redirect('home/login');
			}			
		}
      ...
      function tambah_data(){
			if(($this->u3a == 1)||($this->u3a == 2)){
				$sdata['id_dosen'] = $this->mod_dosennonif->get_id_dosen();
				$sdata['query'] = $this->mod_dosennonif->get_all_dosen();
				$sdata['msg'] = $this->session->flashdata('message');
				$data['main_content'] = $this->load->view('dosennonif/form_add',$sdata,TRUE);
				$data['sub'] = 7;
      ...
      function view_dosennonif(){
		$this->tambah_data();
	  }
      function update_data($id_dosen){
			if(($this->u3a == 1)||($this->u3a == 2)){
				$sdata['hasil'] = $this->mod_dosennonif->getById($id_dosen);
				$data['sub'] = 7;
				$data['main_content'] = $this->load->view('dosennonif/update_dosen', $sdata, TRUE);
				$this->load->view('template/template',$data);
			}else{
					redirect('home');
			}	
      ...
      function save_update_data($id_dosen){
		...
		function save_tambahdata(){
			   ...
			   $jabatan = $this->input->post('jabatan');
				...
				$this->mod_dosennonif->tambah_dosen($inp);
      ...
      function delete_data($id_dosen){
			if(($this->u3a == 1)||($this->u3a == 2)){
				$this->mod_dosennonif->delete_data($id_dosen);
		...
		function upload()
		 {
			...
			else
			{
				...
				$this->mod_dosennonif->upload_data($excel_data);            
		...
		function _prosesidmax($i){
			$idMax = $this->mod_dosennonif->get_id_dosen(); 
			$noUrut = (int) substr($idMax, 5, 3);
			  
			$noUrut = $noUrut+$i-2;
			 
			$newID = 'NONIF' . sprintf("%03s", $noUrut);
			return $newID;
		}
   }
?>