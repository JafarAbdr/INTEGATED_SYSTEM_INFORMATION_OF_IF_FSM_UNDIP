...
class Con_beban_non_fsm extends CI_Controller{
   public function __construct(){
         parent:: __construct();
			...
			if(ISSET($u1) && ISSET($u2) && ISSET($u3) && ($u1 != "") && ($u2 != "") && ($u3 != "")){
				$this->load->model('mod_beban_non_fsm');    
			}else{
				redirect('home/login');
			}
   }
   ...
   function view_bebannonfsm(){
   	$sdata['q'] = $this->mod_beban_non_fsm->get_beban_for_view();
      $sdata['dos1'] = $this->mod_beban_non_fsm->get_dosen_1();
      $sdata['dos2'] = $this->mod_beban_non_fsm->get_dosen_2();
	  $sdata['msg'] = $this->session->flashdata('message');
      $data['main_content'] = $this->load->view('beban_non_fsm/view_bebannonfsm',$sdata,true);
      $data['sub'] = 6;
      $this->load->view('template/template',$data);
   }
	function delete_data($id_beban_non_fsm){
		if(($this->u3a == 1)||($this->u3a == 2)){
			$this->mod_beban_non_fsm->delete_data($id_beban_non_fsm);
			$this->session->set_flashdata('message', "<div class='badge badge-success'>Data Berhasil Dihapus</div>");
			redirect('beban_non_fsm/add');
		}else {
			redirect('home/');
		}
	}
	function update_data($id_beban_non_fsm){
		if(($this->u3a == 1)||($this->u3a == 2)){
			$sdata = $this->_valueUpdate($id_beban_non_fsm);
			$data['main_content'] = $this->load->view('beban_non_fsm/form_bebannonfsm',$sdata,TRUE);
			$data['sub'] = 6;
			$this->load->view('template/template',$data);      
		}else {
			redirect('home/');
	...
	function save_update_data($id_beban_non_fsm){
      ...
      //}else{    
         ...
         //print_r($data);
         $this->mod_beban_non_fsm->update($id_beban_non_fsm,$data);
   ...
   function validate_tambahdata(){
     ...
     // }else{
         ...
         $this->mod_beban_non_fsm->tambah_data($inp);
   ...
   function _valueUpdate($id_beban_non_fsm){
      $q = $this->mod_beban_non_fsm->get_beban_by_id($id_beban_non_fsm);
         
      //id
      $data['id_beban_non_fsm'] = $q->id_beban_non_fsm;
      ...
      if($dosen2!=$dosen1){
		...
		$qd = $this->mod_beban_non_fsm->get_id_nama_dosen();
		$dropdown[''] = '';
		foreach($qd as $hasil){
   ...
   /*-- fungsi untuk view tambah data --*/
	function _valueTambah(){
      ...
      if($month<=6){
		...
		$dropdown= null;
		$q = null;
		$q = $this->mod_beban_non_fsm->get_id_nama_dosen();
      $dropdown[''] = '';
      foreach($q as $hasil){
         $dropdown[$hasil->id_dosen] = $hasil->nama_dosen;
      }
   ...
   function _formvalidation(){
      ...
      $this->form_validation->set_rules('semesterke','Semester Ke','required');
