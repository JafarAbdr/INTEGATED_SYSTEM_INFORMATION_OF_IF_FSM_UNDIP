<?php
	class Con_report_SKKelebihan extends CI_Controller{
	   public function __construct(){
         parent::__construct();
         $this->load->model('mod_report_skkelebihan');
      }
		function index(){
   ...
   function _get_nama_dosen($thn_ajar,$semester){
      ...
      $temp = $this->mod_report_skkelebihan->get_distinct_dosen_1($thn_ajar,$semester);
      if(!isset($temp)){
         return;
      }else{
      //print_r($temp);
      ...
      //get dosen 2
      //simpan ke dalam array
      $temp = $this->mod_report_skkelebihan->get_distinct_dosen_2($thn_ajar,$semester);
      $i = 0;
      $d2 = array();
      foreach($temp as $hasil){
         $d2[$i] = $hasil->dosen_2;
      ...
      for($inc=0;$inc<count($dosen);$inc++){
         $q = $this->mod_report_skkelebihan->get_nama_nip_pangkat_jabatan_dosen_by_id($dosen[$inc]);   
   ...
   function _get_kelas_nama_matkul($id_dosen,$thn_ajar,$semester){
      ...
      $temp = $this->mod_report_skkelebihan->get_kelas_matkul_by_id_dosen($id_dosen,$thn_ajar,$semester);
      //echo count($temp);
     // echo '<br/>';
     // print_r($temp);
      //echo '<br/>';
      ...
      for($i=0;$i<$m;$i++){
         ...
         $kelas = '(';
			...
			$kelas .= ')';
         //$jumlah_matkul += $n;
         $q1 = $this->mod_report_skkelebihan->get_nama_sks_semesterke_matkul_by_id($temp[$i]['matkul']);
         $id_pengampu = $temp[$i]['id_pengampu'];
         $q2 = $this->mod_report_skkelebihan->get_dosen1_dosen2_by_id_pengampu($id_pengampu);
	...
	function _get_nama_dosen_fsm($thn_ajar,$semester){
		$temp = $this->mod_report_skkelebihan->get_distinct_dosen_1_fsm($thn_ajar,$semester);
      if(!isset($temp)){
         return;
      }else{
      //print_r($temp);
      ...
      //get dosen 2
      //simpan ke dalam array
      $temp = $this->mod_report_skkelebihan->get_distinct_dosen_2_fsm($thn_ajar,$semester);
      $i = 0;
      $d2 = array();
      foreach($temp as $hasil){
         $d2[$i] = $hasil->dosen_2;
	...
	function _get_kelas_nama_matkul_fsm($id_dosen,$thn_ajar,$semester){
      $array_alphabet = range('A','Z');
      //print_r($array_alphabet);
      $temp = $this->mod_report_skkelebihan->get_kelas_matkul_by_id_dosen_fsm($id_dosen,$thn_ajar,$semester);
      //echo count($temp);
     // echo '<br/>';
     // print_r($temp);
      //echo '<br/>';
      ...
      for($i=0;$i<$m;$i++){
         ...
         $q2 = $this->mod_report_skkelebihan->get_dosen1_dosen2_by_id_beban_fsm($id_pengampu);
   ...
   function _get_drop_down(){
      ...
      $q = $this->mod_report_skkelebihan->get_thn_ajar_pengampu();
      foreach($q as $hasil){
