<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
   class Con_dosenif extends CI_Controller{
		public function __construct(){
			...
			if(ISSET($u1) && ISSET($u2) && ISSET($u3) && ($u1 != "") && ($u2 != "") && ($u3 != "")){
				$this->load->model('mod_dosenif'); 
			}else{
				redirect('home/login');
			}			
		}
      function view_dosenif(){
        $this->tambah_data();         
      }
      
      function tambah_data(){
			if(($this->u3a == 1)||($this->u3a == 2)){
				$sdata['id_dosen'] = $this->mod_dosenif->get_id_dosen();
				$sdata['query'] = $this->mod_dosenif->get_all_dosen();
				$sdata['msg'] = $this->session->flashdata('message');
				$data['main_content'] = $this->load->view('dosenif/form_add',$sdata,TRUE);
				$data['sub'] = 2;
      ...
      function update_data($id_dosen){
			if(($this->u3a == 1)||($this->u3a == 2)){
				$sdata['hasil'] = $this->mod_dosenif->getById($id_dosen);
				$data['sub'] = 2;
				$data['main_content'] = $this->load->view('dosenif/update_dosen', $sdata, TRUE);
				$this->load->view('template/template',$data);
			}else{
					redirect('home');
			}	
      ...
      function save_update_data($id_dosen){
		...
		function save_tambahdata(){
			   ...
			   $jabatan = $this->input->post('jabatan');
				...
				$this->mod_dosenif->tambah_dosen($inp);
      ...
      function delete_data($id_dosen){
			if(($this->u3a == 1)||($this->u3a == 2)){
				$this->mod_dosenif->delete_data($id_dosen);
		...
		function upload()
		 {
			...
			else
			{
				...
				$this->mod_dosenif->upload_data($excel_data); 
		...
		function _prosesidmax($i){
			$idMax = $this->mod_dosenif->get_id_dosen();
