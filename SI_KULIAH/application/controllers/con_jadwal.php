<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	class Con_jadwal extends CI_Controller{
		...
		function index(){
			$sdata['q'] = $this->mod_jadwal->get_jadwal_for_view();
		...
		function delete_data($id_jadwal){
			if(($this->u3a == 1)||($this->u3a == 2)){
				$this->mod_jadwal->delete_data($id_jadwal);
		...
		function update_data_manual($id_jadwal){
			...
			//echo $sdata['hasil'];
				...
				$sdata['hasil'] = $this->mod_jadwal->get_jadwal_by_id($id_jadwal);
				$sdata['msg'] = $this->session->flashdata('message');
				$data['sub'] = 4;
				$sdata['id_jadwal'] = $id_jadwal;
				$data['main_content'] = $this->load->view('jadwal/update_jadwal_manual',$sdata,TRUE);
				$this->load->view('template/template',$data);      
			}else{
					redirect('home');
			}
		...
		function update_data_auto($id_jadwal){
			...
			//echo $sdata['hasil'];
				...
				$sdata['id_jadwal'] = $id_jadwal;
				$data['sub'] = 4;
				$data['main_content'] = $this->load->view('jadwal/update_jadwal',$sdata,TRUE);
				$this->load->view('template/template',$data);      
			}else{
					redirect('home');
			}
		...
		function save_update_data_manual($id_jadwal){
				...
				$q = $this->mod_jadwal->get_all_pengampu_by_id($id_pengampu);
				$matkul = $q->matkul;
				$dosen_1 = $q->dosen_1;
				$dosen_2 = $q->dosen_2;
				$kel = $q->jumlah_kelas;
	...
	//			print_r($kelas);
				$query = $this->mod_jadwal->cek_sama_id($id_jadwal, $thn_ajar, $semester, $hari, $jammulai, $jamselesai, $ruang, $matkul, $dosen_1, $dosen_2);
				//print_r($query);
				$inp = array(
					'thn_ajar' => $thn_ajar,
					'semester' => $semester,
				...
				if($temp['bool']){
					...
					$sdata['inp'] = $inp;
					$sdata['id_jadwal'] = $id_jadwal;
					$data['main_content'] = $this->load->view('jadwal/update_jadwal_manual_2',$sdata,TRUE);
					$data['sub'] = 4;
					$this->load->view('template/template',$data);
				}else{
					$this->mod_jadwal->update($id_jadwal, $inp);
	...
	//			print_r($kelas);
				...
				if($temp['bool']){
					...
					$sdata['inp'] = $inp;
					$sdata['id_jadwal'] = $id_jadwal;
					$data['main_content'] = $this->load->view('jadwal/update_jadwal_2',$sdata,TRUE);
					$data['sub'] = 4;
					$this->load->view('template/template',$data);
				}else{
					$this->mod_jadwal->update($id_jadwal, $inp);
		...
		function tambahdata_manual(){
			if(($this->u3a == 1)||($this->u3a == 2)){
				...
				$sdata['dos1'] = $this->mod_jadwal->get_dosen_1();
				$sdata['dos2'] = $this->mod_jadwal->get_dosen_2();
				$sdata['msg'] = $this->session->flashdata('message');
				$data['main_content'] = $this->load->view('jadwal/tambah_jadwalmanual',$sdata,TRUE);
				$data['sub'] = 4;
				$this->load->view('template/template',$data);
			}else{
					redirect('home');
		...
		function validate_tambahdatamanual(){
				...
				//print_r($kelas);
				$query = $this->mod_jadwal->cek_sama($thn_ajar, $semester, $hari, $jammulai, $jamselesai, $ruang, $matkul, $dosen_1, $dosen_2);
				//print_r($query);
				$inp = array(
					'thn_ajar' => $thn_ajar,
					'semester' => $semester,
				...
				if($temp['bool']){
					...
					$sdata['temp'] = $temp;
					$sdata['inp'] = $inp;
					$data['main_content'] = $this->load->view('jadwal/tambah_jadwalmanual_2',$sdata,TRUE);
					$data['sub'] = 4;
					$this->load->view('template/template',$data);
				}else{
					$this->mod_jadwal->tambah_data($inp);
					echo '<script>alert("Success")</script>';
		...
		function tambahdata_auto(){
			if(($this->u3a == 1)||($this->u3a == 2)){
				...
				$sdata['dos2'] = $this->mod_jadwal->get_dosen_2();
				$sdata['msg'] = $this->session->flashdata('message');
				$data['main_content'] = $this->load->view('jadwal/tambah_jadwal',$sdata,TRUE);
				$data['sub'] = 4;
				$this->load->view('template/template',$data);
			}else{
					redirect('home');
		...
		function validate_tambahdata(){
				...
				if($temp['bool']){
					...
					$sdata['temp'] = $temp;
					$sdata['inp'] = $inp;
					$data['main_content'] = $this->load->view('jadwal/tambah_jadwal_2',$sdata,TRUE);
					$data['sub'] = 4;
					$this->load->view('template/template',$data);
				}else{
					$this->mod_jadwal->tambah_data($inp);
		...
		function _cek_kesamaan($query, $inp){
			foreach ($query as $hasil) {
				...
				if(($jam1<=$jam2)&&($jam3>=$jam4)){
					$return['bool'] = true;
					$return['salah'] = 'jam kuliah';
					return $return;
				}
				if(($jam1<=$jam2)&&($jam3>=$jam2)){
					$return['bool'] = true;
					$return['salah'] = 'jam kuliah';
					return $return;
				}
		...
		function _valueUpdate($id_jadwal){
			$q = $this->mod_jadwal->get_jadwal_by_id($id_jadwal);
			//print_r($q);	
			//id
			...
			$semester = $q->semester;
			$pengampu = $q->pengampu;
			$q1 = $this->mod_jadwal->get_id_nama_matkul_from_pengampu($thn_ajar, $semester);
			//$q = $this->mod_jadwal->get_id_nama_matkul();
			$dropdown[''] = '';
			foreach($q1 as $hasil){
				$dropdown[$hasil->id_pengampu] = $this->mod_jadwal->get_nama_matkul_by_id($hasil->matkul)->nama_matkul;
			}

			$tempo = $this->mod_jadwal->get_matkul_from_pengampu_by_id($pengampu)->matkul;
			//print_r($pengampu);
			$dropdown[$pengampu] = $this->mod_jadwal->get_nama_matkul_by_id($tempo)->nama_matkul;
			...
			//echo $q->ruang;
			$dropdown = null;
			$qr = $this->mod_jadwal->get_id_nama_ruang();
			$dropdown[''] = '';
			foreach($qr as $hasil){
		...
		function _valueupdatemanual($id_jadwal){
			...
			//tahun ajar
			$tahun_ajar = $q->thn_ajar;
			$qus = $this->mod_jadwal->get_thnajar_from_pengampu();
		...
		function _valueTambah(){
			...
			$thn_ajar = $this->session->userdata('thn_ajar');
			$semester = $this->session->userdata('semester');
			$q = $this->mod_jadwal->get_id_nama_matkul_from_pengampu($thn_ajar, $semester);
			//$q = $this->mod_jadwal->get_id_nama_matkul();
			$dropdown[''] = '';
			foreach($q as $hasil){
				$dropdown[$hasil->id_pengampu] = $this->mod_jadwal->get_nama_matkul_by_id($hasil->matkul)->nama_matkul;
		...
		//	print_r($dropdown);
			//dropdown ruang
			$dropdown = null;
			$q = $this->mod_jadwal->get_id_nama_ruang();
			$dropdown[''] = '';
			foreach($q as $hasil){
				$dropdown[$hasil->id_ruang] = $hasil->nama_ruang;
			}
		...
		function _valuetambahmanual(){
			//dropdown YEar
			$q = $this->mod_jadwal->get_thnajar_from_pengampu();
			//print_r($q);
			//$dropdown[''] = '';
			foreach ($q as $hasil) {
				$dropdown[$hasil->thn_ajar] = $hasil->thn_ajar;
		...
		function get_semester(){
			...
			$q = $this->mod_jadwal->get_semester_by_thn_ajar($thn_ajar);
			//$i=0;
			foreach($q as $hasil){
		...
		function get_all_pengampu(){
			...
			foreach($q as $hasil){
				$a = $hasil->id_pengampu;
				$b = $this->mod_jadwal->get_nama_matkul_by_id($hasil->matkul)->nama_matkul;
