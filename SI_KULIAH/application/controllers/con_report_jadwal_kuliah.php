...
class Con_report_jadwal_kuliah extends CI_Controller{
   ...
   function index(){
      $sdata['q'] = $this->mod_report_jadwal_kuliah->get_jadwal_for_view();
		$data['sub'] = 10;
      $data['main_content'] = $this->load->view('report/jadwalkuliah/view_jadwal',$sdata,true);
	...
	function jadwal(){
      ...
      //Load View
      $data['main_content'] = $this->load->view('report/jadwalkuliah/form_jadwal',$sdata,TRUE);
      $data['sub'] = 10;
      $this->load->view('template/template',$data);
   }   
      ...
      $semester = '\''.$semester.'\'';
		...
		if(!isset($hari) or empty($hari)){
         $sdata['hasil_report'] = '
			...
			//print_r($tabel_hari['hari']);
			$sdata['tabel_hari'] = $tabel_hari;
			$sdata['jadwal'] = $jadwal;
			$ssdata['table_jadwal'] = $this->load->view('report/jadwalkuliah/table_jadwal',$sdata, TRUE);
			$sssdata['hasil_report'] = $this->load->view('report/jadwalkuliah/view_jadwal',$ssdata, TRUE);
			$data['main_content'] = $this->load->view('report/jadwalkuliah/form_jadwal',$sssdata,TRUE);
			$data['sub'] = 10;
			$this->load->view('template/template',$data);
		}
   ...
   function _getDropDown(){
      ...
      $q = $this->mod_report_jadwal_kuliah->get_thn_ajar_jadwal();
      foreach($q as $hasil){
